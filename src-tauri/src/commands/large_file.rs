use crate::models::file_meta::ChunkInfo;
use crate::services::chunk_reader::ChunkReader;
...
pub fn open_large_file(path: String) -> Result<LargeFileInitResult, String> {
    let reader = ChunkReader::new(&path, None).map_err(|e| friendly("读取文件", &e))?;
    let file_size = reader.file_size();
    let total_chunks = reader.total_chunks();

    let first_chunk_bytes = reader.read_chunk(0).map_err(|e| friendly("读取文件", &e))?;
    ...
    let chunk_info = ChunkInfo {
        ...
        total_chunks,
        start_offset: 0,
        end_offset: first_chunk_bytes.len() as u64,
        line_start: 1,
        line_end: line_start,
        content,
...
pub fn read_chunk(path: String, chunk_index: u64) -> Result<ChunkInfo, String> {
    ...
    let end_offset = start_offset + chunk_bytes.len() as u64;
...
pub fn count_lines(path: String) -> Result<u64, String> {
    let reader = ChunkReader::new(&path, None).map_err(|e| friendly("读取文件", &e))?;
    reader.count_lines().map_err(|e| friendly("读取文件", &e))
}
