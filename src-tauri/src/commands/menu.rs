...
pub fn build_menu(app_handle: &AppHandle, lang: &str) -> tauri::Result<()> {
    let m = |id: &str, label: &'static str| MenuItem::with_id(app_handle, id, label, true, None::<&str>);

    let file_menu = Submenu::with_items(app_handle, tr(lang, "文件", "File"), true, &[
        &m("new", tr(lang, "新建", "New"))?, &m("open", tr(lang, "打开...", "Open..."))?, &m("open_with_encoding", tr(lang, "按编码打开...", "Open with Encoding..."))?,
    ...
    let edit_menu = Submenu::with_items(app_handle, tr(lang, "编辑", "Edit"), true, &[
        &m("edit_undo", tr(lang, "撤销", "Undo"))?, &m("edit_redo", tr(lang, "重做", "Redo"))?,
        &m("edit_cut", tr(lang, "剪切", "Cut"))?, &m("edit_copy", tr(lang, "复制", "Copy"))?, &m("edit_paste", tr(lang, "粘贴", "Paste"))?,
    ...
    let search_menu = Submenu::with_items(app_handle, tr(lang, "查找", "Search"), true, &[
        &m("find", tr(lang, "查找...", "Find..."))?, &m("find_next", tr(lang, "查找下一个", "Find Next"))?, &m("find_prev", tr(lang, "查找上一个", "Find Previous"))?,
        &m("replace", tr(lang, "替换...", "Replace..."))?,
        &m("find_in_files", tr(lang, "在文件中查找...", "Find in Files..."))?, &m("batch_find_replace", tr(lang, "批量查找替换...", "Batch Find/Replace..."))?, &m("multi_search", tr(lang, "多文档查找替换...", "Multi-Document Search..."))?,
        &m("goto", tr(lang, "转到行...", "Go to Line..."))?, &m("jump_to_bracket", tr(lang, "跳转到匹配括号", "Jump to Bracket"))?, &m("select_to_bracket", tr(lang, "选中到匹配括号", "Select to Bracket"))?,
        &m("mark_all", tr(lang, "标记所有匹配", "Mark All Matches"))?, &m("unmark_all", tr(lang, "取消所有标记", "Unmark All"))?,
        &m("next_bookmark", tr(lang, "下一书签", "Next Bookmark"))?, &m("prev_bookmark", tr(lang, "上一书签", "Previous Bookmark"))?, &m("clear_bookmarks", tr(lang, "清除所有书签", "Clear All Bookmarks"))?,
    ])?;

    let view_menu = Submenu::with_items(app_handle, tr(lang, "视图", "View"), true, &[
        &m("toggle_sidebar", tr(lang, "切换侧边栏", "Toggle Sidebar"))?, &m("command_palette", tr(lang, "命令面板...", "Command Palette..."))?,
        &m("split_horizontal", tr(lang, "水平分屏", "Split Horizontal"))?, &m("split_vertical", tr(lang, "垂直分屏", "Split Vertical"))?, &m("split_close", tr(lang, "关闭分屏", "Close Split"))?,
        &m("function_list", tr(lang, "函数列表...", "Function List..."))?, &m("doc_switcher", tr(lang, "切换文档...", "Switch Document..."))?,
    ...
    let encoding_menu = Submenu::with_items(app_handle, tr(lang, "编码", "Encoding"), true, &[
        ...
        &m("encode_utf8", tr(lang, "用 UTF-8 编码", "Encode as UTF-8"))?, &m("encode_utf8_bom", tr(lang, "用 UTF-8-BOM 编码", "Encode as UTF-8-BOM"))?,
        &m("encode_gbk", tr(lang, "用 GBK 编码", "Encode as GBK"))?, &m("encode_gb2312", tr(lang, "用 GB2312 编码", "Encode as GB2312"))?,
        &m("encode_utf16le", tr(lang, "用 UTF-16LE 编码", "Encode as UTF-16LE"))?, &m("encode_utf16be", tr(lang, "用 UTF-16BE 编码", "Encode as UTF-16BE"))?,
        &m("encode_ascii", tr(lang, "用 ASCII 编码", "Encode as ASCII"))?,
        &m("convert_utf8", tr(lang, "转换为 UTF-8", "Convert to UTF-8"))?, &m("convert_utf8_bom", tr(lang, "转换为 UTF-8-BOM", "Convert to UTF-8-BOM"))?,
        &m("convert_gbk", tr(lang, "转换为 GBK", "Convert to GBK"))?, &m("convert_gb2312", tr(lang, "转换为 GB2312", "Convert to GB2312"))?,
        &m("convert_utf16le", tr(lang, "转换为 UTF-16LE", "Convert to UTF-16LE"))?, &m("convert_utf16be", tr(lang, "转换为 UTF-16BE", "Convert to UTF-16BE"))?,
    ])?;

    let language_menu = Submenu::with_items(app_handle, tr(lang, "语言", "Language"), true, &[
        &m("language_selector", tr(lang, "选择语言...", "Select Language..."))?,
    ...
    let settings_menu = Submenu::with_items(app_handle, tr(lang, "设置", "Settings"), true, &[
        ...
        &m("shortcut_mapper", tr(lang, "快捷键映射...", "Shortcut Mapper..."))?,
        &m("shortcuts", tr(lang, "快捷键帮助...", "Shortcut Help..."))?,
        &m("snippets", tr(lang, "代码片段...", "Snippets..."))?,
        &m("clipboard_history", tr(lang, "剪贴板历史...", "Clipboard History..."))?,
        &m("plugin_manager", tr(lang, "插件管理...", "Plugin Manager..."))?,
    ])?;

    let tools_menu = Submenu::with_items(app_handle, tr(lang, "工具", "Tools"), true, &[
        &m("char_stats", tr(lang, "字符统计...", "Character Stats..."))?, &m("hex_viewer", tr(lang, "十六进制查看...", "Hex Viewer..."))?,
        &m("text_transform", tr(lang, "文本转换...", "Text Transform..."))?,
        &m("macro_start_stop", tr(lang, "开始/停止录制宏", "Start/Stop Macro Recording"))?,
        &m("macro_playback", tr(lang, "播放宏", "Playback Macro"))?,
        &m("run_macro_multiple", tr(lang, "多次运行宏...", "Run Macro Multiple..."))?,
        &m("macro_save", tr(lang, "保存当前录制的宏", "Save Current Macro"))?,
    ])?;
    ...
    let compare_menu = Submenu::with_items(app_handle, tr(lang, "对比", "Compare"), true, &[
        ...
        &m("compare_prev_diff", tr(lang, "上一差异", "Previous Difference"))?,
    ])?;
