...
impl ChunkReader {
    pub fn new(path: &str, chunk_size: Option<usize>) -> io::Result<Self> {
        ...
        let total_chunks = if file_size == 0 {
            ...
            (file_size as usize).div_ceil(chunk_size) as u64
        };
    ...
    pub fn read_chunk(&self, chunk_index: u64) -> io::Result<Vec<u8>> {
        ...
        if self.file_size > self.chunk_size as u64 * 2 {
    ...
    pub fn read_tail(&self, tail_bytes: u64) -> io::Result<Vec<u8>> {
        if self.file_size <= tail_bytes {
            return self.read_chunk(0);
        }
        ...
        for (i, &b) in mmap.iter().enumerate() {
            if b == b'\n' {
                ...
                if current_line == target_line {
                    let line_bytes = &mmap[last_pos..i];
                    return Ok((
                        current_line,
...
pub fn read_file_content(path: &str) -> io::Result<Vec<u8>> {
    std::fs::read(path)
}
